...
type TunService struct {
	...
	// PoolRef 绑定的线路池 ID（可选）。设置后 TunService 的候选线路由池内
	// 线路（含远程节点入口）构成；空 = 沿用默认 LineRefs 集合，行为不变。
	// PoolRef 仅作为 hint，实际池管理随多节点 Master（#89）合入后进一步扩展。
	PoolRef string `gorm:"size:64" json:"pool_ref"`
}
