...
// 并在窗口超时后自动恢复。单实例内存态（重启即清零）；对多实例部署仅提供
// 实例级限制，足够覆盖面板典型的单实例家庭场景。
package ratelimit
...
// IPLimiter 固定窗口计数限流器（按 key，通常传客户端 IP）。
...
func NewLimiter(window time.Duration, max int) *IPLimiter {
	return &IPLimiter{window: window, max: max, counts: make(map[string]*windowCount)}
}
